//
//  DataDetailMonthController.swift
//  KaiFangBao
//
...
class DataDetailMonthController: DataDetailBaseController, UITableViewDelegate, UITableViewDataSource {
    ...
    override func fetchProjectDataItem() -> Void{
        ...
        let firstDayOfMonthString = NSDate().fs_firstDayOfMonth.fs_stringWithFormat("yyyy-MM-dd")
        let lastDayOfMonthString = NSDate().fs_lastDayOfMonth.fs_stringWithFormat("yyyy-MM-dd")
        self.networkManager.GET(url, parameters: ["startDate":firstDayOfMonthString,"endDate":lastDayOfMonthString,"itemId":"\(self.itemId)"], progress: nil, success: {[unowned self] (task, result) in
