...
class WeekReportController: OperationDataReportBaseController ,UITableViewDelegate, UITableViewDataSource, KFBDateSelectorDelegate {
    ...
    func configureDateSelector() -> Void{
        self.dateSelector = KFBDateSelector(type: .week)
    ...
    override func fetchProjectReportData() -> Void{
        ...
        let firstDayOfWeekString = self.currentDate.fs_firstDayOfWeek.fs_stringWithFormat("yyyy-MM-dd")
        let lastDayOfWeekString = self.currentDate.fs_firstDayOfWeek.fs_dateByAddingDays(6).fs_stringWithFormat("yyyy-MM-dd")
        self.networkManager.GET(url, parameters: ["startDate":firstDayOfWeekString,"endDate":lastDayOfWeekString], progress: nil, success: {[unowned self] (task, result) in
