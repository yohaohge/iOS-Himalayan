...
protocol OperationDataReportBaseControllerDelegate:NSObjectProtocol {
    
}

class OperationDataReportBaseController: BaseController {
    weak var delegate:OperationDataReportBaseControllerDelegate!
    var houseId:Int!
    var shouldUpdateData:Bool = true
    var refreshHeader:MJRefreshNormalHeader!
    func fetchProjectReportData() -> Void{}
    func updateDataIfNeeded() -> Void{}
}

class DayReportController: OperationDataReportBaseController ,UITableViewDelegate, UITableViewDataSource, KFBDateSelectorDelegate, UIViewControllerTransitioningDelegate,KFBPopCalendarControllerDelegate {
    ...
    func configureDateSelector() -> Void{
        self.dateSelector = KFBDateSelector(type: .day)
    ...
    func didClickDayLabel(date: NSDate) {
    ...
    override func fetchProjectReportData() -> Void{
        ...
        self.networkManager.GET(url, parameters: ["startDate":dateString,"endDate":dateString], progress: nil, success: {[unowned self] (task, result) in
